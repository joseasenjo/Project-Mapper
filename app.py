...
from project_mapper_core import (
    analyze_project, format_size, extract_zip_from_bytes,
    download_from_url, compare_projects, generate_pdf_report
)

st.set_page_config(page_title="Project Mapper", page_icon="📂", layout="wide")

# Cargar tema desde config.toml (ya está)
...
with st.sidebar:
    ...
    2. O introduce una URL de Google Drive/Dropbox.
    3. O usa el proyecto de ejemplo para probar.
    4. Obtén un informe completo con gráficos y descarga PDF.
    """)
    st.markdown("---")
    st.markdown("### 🚀 Comparativa")
    st.markdown("Sube dos proyectos y compara sus métricas.")

# Tabs
tab1, tab2, tab3 = st.tabs(["📊 Análisis", "⚖️ Comparativa", "🌐 Remoto"])
...
with tab1:
    ...
    with col2:
        if st.button("📁 Usar proyecto de ejemplo", key="example_analysis"):
            with st.spinner("Generando proyecto de ejemplo..."):
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    with open(os.path.join(tmpdir, 'docs', 'manual.pdf'), 'wb') as f:
                        f.write(b'%PDF-1.4%' + b'\x00' * 1024 * 100)  # 100 KB
    ...
    if 'data' in st.session_state and 'error' not in st.session_state['data']:
        ...
        with col1:
            ...
            st.download_button("📄 JSON", json_str, "informe_proyecto.json", "application/json")
        with col2:
            # Generar PDF
                ...
                with st.spinner("Generando PDF..."):
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp_pdf:
                        ...
                        with open(tmp_pdf.name, 'rb') as f:
                            pdf_bytes = f.read()
                        st.download_button("📥 Descargar PDF", pdf_bytes, "informe_proyecto.pdf", "application/pdf")
        with col3:
            ...
            txt_report = "\n".join(lines)
            st.download_button("📝 TXT resumen", txt_report, "resumen_proyecto.txt", "text/plain")

# ==================== TAB 2: COMPARATIVA ====================
with tab2:
    st.header("⚖️ Comparativa de proyectos")
    col1, col2 = st.columns(2)
    data1 = None
    data2 = None
    with col1:
        uploaded1 = st.file_uploader("📤 Proyecto 1 (ZIP)", type=['zip'], key="comp1")
        if uploaded1 is not None:
            with st.spinner("Analizando proyecto 1..."):
    ...
    with col2:
        uploaded2 = st.file_uploader("📤 Proyecto 2 (ZIP)", type=['zip'], key="comp2")
        if uploaded2 is not None:
            with st.spinner("Analizando proyecto 2..."):
    ...
    if data1 and data2:
        ...
        col1, col2, col3 = st.columns(3)
        col1.metric("Tamaño Proyecto 1", data1['size_human'])
        col2.metric("Tamaño Proyecto 2", data2['size_human'])
        ...
        st.write("**Distribución de extensiones (comparativa):**")
        ...
        for ext in all_exts:
            ...
            comp_data.append({'Extensión': ext, 'Proyecto1 (KB)': size1/1024, 'Proyecto2 (KB)': size2/1024})
        df_comp = pd.DataFrame(comp_data)
        st.dataframe(df_comp, use_container_width=True)

        # Gráfico comparativo de tamaño por extensión
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df_comp['Extensión'], y=df_comp['Proyecto1 (KB)'], name='Proyecto 1'))
        fig.add_trace(go.Bar(x=df_comp['Extensión'], y=df_comp['Proyecto2 (KB)'], name='Proyecto 2'))
        fig.update_layout(barmode='group', title='Comparativa de tamaño por extensión (KB)')
        st.plotly_chart(fig, use_container_width=True)

# ==================== TAB 3: REMOTO ====================
with tab3:
    st.header("🌐 Analizar proyecto desde URL remota")
    st.write("Introduce un enlace de Google Drive o Dropbox (compartido) para analizar un proyecto ZIP.")

    remote_url = st.text_input("🔗 URL del archivo ZIP (Google Drive / Dropbox)", key="remote_url")
    if st.button("📥 Descargar y analizar", key="remote_analyze"):
        if remote_url:
    ...
    if 'data' in st.session_state and 'error' not in st.session_state['data']:
        # Reutilizar la visualización de datos de la tab1 (podríamos mostrarlo aquí también)
        # Pero para no repetir, mostramos un mensaje diciendo que vaya a la pestaña Análisis
        st.info("Los resultados se muestran en la pestaña 'Análisis'. Ve allí para ver el informe completo.")
